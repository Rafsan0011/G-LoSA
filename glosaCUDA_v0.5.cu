...
void help(string msg){
	cout << msg
		...
		<< "    -iter    the number of iteration for maximum clique search (1-3, default=3)                      " << "\n"
...
// the kernel that does the most intensive calculations' part
__global__ void calc_Distances(float3 *vRefStruct_Rv, float3* vFitStruct_Rv, int2 *vPair_Rv, bool *dev_dist, 
							int num_pairs, int vRefSize, int vFitSize, float dist_tol_cutoff)
{
	// take advantage of GPU shared memory, much faster during the loop execution
	// because of many "global" array accesses
	__shared__ float3 vRefStruct_Sh[200];
	__shared__ float3 vFitStruct_Sh[200];
	__shared__ int2 vPair_Sh[600];

	// fetch elements from GPU global memory
	for (int i = threadIdx.x; i < vRefSize; i += blockDim.x)
		vRefStruct_Sh[i] = vRefStruct_Rv[i];
	for (int i = threadIdx.x; i < vFitSize; i += blockDim.x)
		vFitStruct_Sh[i] = vFitStruct_Rv[i];
	for (int i = threadIdx.x; i < num_pairs; i += blockDim.x)
		vPair_Sh[i] = vPair_Rv[i];

	__syncthreads(); 

	float d_dist, dist_1, dist_2;

	for (int i = blockIdx.x; i < num_pairs; i+=gridDim.x)
	{
		// take advantage of Nvidia's GPU function norm3df() 
		for (int j = i + 1 + threadIdx.x; j < num_pairs; j+=blockDim.x)
		{
			dist_1 = norm3df(vRefStruct_Sh[vPair_Sh[i].x].x - vRefStruct_Sh[vPair_Sh[j].x].x, 
					vRefStruct_Sh[vPair_Sh[i].x].y - vRefStruct_Sh[vPair_Sh[j].x].y,
					vRefStruct_Sh[vPair_Sh[i].x].z - vRefStruct_Sh[vPair_Sh[j].x].z);

			dist_2 = norm3df(vFitStruct_Sh[vPair_Sh[i].y].x - vFitStruct_Sh[vPair_Sh[j].y].x,
					vFitStruct_Sh[vPair_Sh[i].y].y - vFitStruct_Sh[vPair_Sh[j].y].y,
					vFitStruct_Sh[vPair_Sh[i].y].z - vFitStruct_Sh[vPair_Sh[j].y].z);

			d_dist = dist_1 - dist_2; 

			if (d_dist < dist_tol_cutoff && d_dist > -dist_tol_cutoff)
				dev_dist[i*num_pairs + j] = TRUE;
			else
				dev_dist[i*num_pairs + j] = FALSE;
		}
	}

}

// this version of GenerateEdges accepts GPU device pointers and calls the correspdoing CUDA kernel
void GenerateEdges(vector< Atom * > &vRefStruct, vector< Atom * > &vFitStruct, vector< Pair * > &vPair, vector< Edge * > &vEdge, float dist_tol_cutoff,
	float3 *d_vRefStruct_Rv, float3* d_vFitStruct_Rv, int2 *d_vPair_Rv, bool *dev_dist, bool *h_dev_dist)
{
	Edge *e;
	...
	// choose a configuration of 8 blocks - 128 threads, reasonable for the current vRefStruct.size() - vFitStruct.size()
	// but can be set to be dynamic
	calc_Distances << <8, 128 >> >(d_vRefStruct_Rv, d_vFitStruct_Rv, d_vPair_Rv, dev_dist, num_pairs, vRefStruct.size(), vFitStruct.size(), dist_tol_cutoff);
	...
	cudaMemcpy(h_dev_dist, dev_dist, num_pairs * num_pairs * sizeof(bool), cudaMemcpyDeviceToHost);

	// for now the last part remains the same as before (but this should also be converted finally...)
	for (int i = 0; i < num_pairs; i++)
	{
		for (int j = i + 1; j < num_pairs; j++)
		{
			if (h_dev_dist[i*num_pairs + j])
			{
				e = new Edge();
				e->pair_i = i;
				e->pair_j = j;
...
void GenerateEdges(vector< Atom * > &vRefStruct, vector< Atom * > &vFitStruct, vector< Pair * > &vPair, vector< Edge * > &vEdge, float dist_tol_cutoff)
{
	...
	float3 *vRefStruct_Rv = (float3*)malloc(vRefStruct.size()*sizeof(float3));
	float3 *vFitStruct_Rv = (float3*)malloc(vFitStruct.size()*sizeof(float3));
	int2 *vPair_Rv = (int2*)malloc(vPair.size()*sizeof(int2));
	for (int i = 0; i < num_pairs; i++)
	{
		vPair_Rv[i].x = vPair[i]->ref_index;
		vPair_Rv[i].y = vPair[i]->fit_index;
	}
	for (int i = 0; i < vRefStruct.size(); i++)
	{
		vRefStruct_Rv[i].x = vRefStruct[i]->R[0];
		vRefStruct_Rv[i].y = vRefStruct[i]->R[1];
		vRefStruct_Rv[i].z = vRefStruct[i]->R[2];
	}
	for (int i = 0; i < vFitStruct.size(); i++)
	{
		vFitStruct_Rv[i].x = vFitStruct[i]->R[0];
		vFitStruct_Rv[i].y = vFitStruct[i]->R[1];
		vFitStruct_Rv[i].z = vFitStruct[i]->R[2];
	}
	...
	for (int i = 0; i < num_pairs; i++)
	{
		for (int j = i + 1; j < num_pairs; j++)
		{
			dist_1 = sqrt(pow(vRefStruct_Rv[vPair_Rv[i].x].x - vRefStruct_Rv[vPair_Rv[j].x].x, 2)
				+ pow(vRefStruct_Rv[vPair_Rv[i].x].y - vRefStruct_Rv[vPair_Rv[j].x].y, 2)
				+ pow(vRefStruct_Rv[vPair_Rv[i].x].z - vRefStruct_Rv[vPair_Rv[j].x].z, 2));

			dist_2 = sqrt(pow(vFitStruct_Rv[vPair_Rv[i].y].x - vFitStruct_Rv[vPair_Rv[j].y].x, 2)
				+ pow(vFitStruct_Rv[vPair_Rv[i].y].y - vFitStruct_Rv[vPair_Rv[j].y].y, 2)
				+ pow(vFitStruct_Rv[vPair_Rv[i].y].z - vFitStruct_Rv[vPair_Rv[j].y].z, 2));
...
void setMaxCliqueAlignedNodes(vector< Atom * > &vRefStruct, vector< Atom * > &vFitStruct,
	vector< Atom * > &vRefAligned, vector< Atom * > &vFitAligned, vector< int > &vPairs)
{
	char buffer[100];
	char index[5];
...
void filterAlignedResiduePairs(vector< Atom * > vRefStruct, vector< Atom * > vFitStruct, float cutoff)
{
	...
	for (int i = 0; i < vRefStruct.size(); i++)
	{
		if (alignedRes[i] != -1)
		{
			dist = sqrt(pow(vRefStruct[i]->R[0] - vFitStruct[alignedRes[i]]->R_sup[0], 2) + 
...
int main(int argc, char *argv[]){
	...
	// ==========================================================
	// *** Creating structs and loading data in GPU ***
	//	  the following arrays are suitable for CUDA optimizations
	//	  and the target is to demonstrate speedup for "GenerateEdges()" function
	// ==========================================================
	cudaError_t cudaStatus;

	// intermediate host pointer to load data from the std::vector
	float3 *vRefStruct_Rv = (float3*)malloc(vRefStruct.size()*sizeof(float3));
	float3 *d_vRefStruct_Rv;	// the corresponding pointer in GPU device
	cudaMalloc((void**)&d_vRefStruct_Rv, vRefStruct.size() * sizeof(float3));
	for (int i = 0; i < vRefStruct.size(); i++)	// creation of float3 array from the vectors' elements
	{
		vRefStruct_Rv[i].x = vRefStruct[i]->R[0];
		vRefStruct_Rv[i].y = vRefStruct[i]->R[1];
		vRefStruct_Rv[i].z = vRefStruct[i]->R[2];
	}
	cudaMemcpy(d_vRefStruct_Rv, vRefStruct_Rv, vRefStruct.size() * sizeof(float3), cudaMemcpyHostToDevice);


	// intermediate host pointer to load data from the std::vector
	float3 *vFitStruct_Rv = (float3*)malloc(vFitStruct.size()*sizeof(float3));
	float3 *d_vFitStruct_Rv;	// the corresponding pointer in GPU device
	cudaMalloc((void**)&d_vFitStruct_Rv, vFitStruct.size() * sizeof(float3));
	for (int i = 0; i < vFitStruct.size(); i++)
	{
		vFitStruct_Rv[i].x = vFitStruct[i]->R[0];
		vFitStruct_Rv[i].y = vFitStruct[i]->R[1];
	...
	// transfer to GPU
	cudaMemcpy(d_vFitStruct_Rv, vFitStruct_Rv, vFitStruct.size() * sizeof(float3), cudaMemcpyHostToDevice);

	int2 *vPair_Rv = (int2*)malloc(vPair.size()*sizeof(int2));
	int2 *d_vPair_Rv; // the corresponding pointer in GPU device
	cudaMalloc((void**)&d_vPair_Rv, vPair.size() * sizeof(int2));
	for (int i = 0; i < vPair.size(); i++)
	{
		vPair_Rv[i].x = vPair[i]->ref_index;
		vPair_Rv[i].y = vPair[i]->fit_index;
	}
	cudaMemcpy(d_vPair_Rv, vPair_Rv, vPair.size() * sizeof(int2), cudaMemcpyHostToDevice);

	// this will be used as a container for TRUE-FALSE of the existence of the edge
	bool *dev_dist;
	bool *h_dev_dist = (bool*)malloc(vPair.size() * vPair.size() * sizeof(bool));
	cudaMalloc((void**)&dev_dist, vPair.size() * vPair.size() * sizeof(bool));
	
	//dummy device pointer for vRefSurface vector's elements
	float3 *d_vRefSurface_Rv;
	//	cudaMalloc((void**)&d_vRefSurface_Rv, vRefSurface.size() * sizeof(float3));

	// --- Enf of CUDA preparation
	...
	for (int i = 0; i < num_iter; i++)
	{
		...
		dist_tolerance = dist_tolerance + i * 0.5;

		if (surf_option == 0)
			GenerateEdges(vRefStruct, vFitStruct, vPair, vEdge, dist_tolerance,
				d_vRefStruct_Rv,  d_vFitStruct_Rv, d_vPair_Rv, dev_dist, h_dev_dist);
		else
			GenerateEdges(vRefSurface, vFitStruct, vPair, vEdge, dist_tolerance,
				d_vRefSurface_Rv, d_vFitStruct_Rv, d_vPair_Rv, dev_dist, h_dev_dist);
	...
	// clear CUDA memory (may change location later)
	cudaFree(d_vRefStruct_Rv);
	cudaFree(d_vFitStruct_Rv);
	cudaFree(d_vPair_Rv);
	cudaFree(dev_dist);
